...
export default function UpComing() {
  ...
  const todayTasks = tasks.filter(
    (t) => localDateString(t.duedate) === todayString,
  );
  const tomorrowTasks = tasks.filter(
    (t) => localDateString(t.duedate) === tomorrowString,
  );
    ...
    list.length === 0 ? (
      <div className="h-full flex flex-col items-center justify-center">
        ...
        <p className="text-gray-400 text-lg">
          No goals yet. Great things start small.
        </p>
      </div>
    ) : (
  ...
  return (
    <div className="flex flex-col">
      <h1 className="font-bold mb-6" style={{ fontSize: 45 }}>
        Upcoming
      </h1>

      {/* Today Section */}
      <div className="border-2 w-[980px] h-[320px] rounded-4xl p-6 flex flex-col">
        <h2 className="font-bold mb-4" style={{ fontSize: 32 }}>
          Today
        </h2>
        <div className="flex-1 overflow-hidden">{renderTasks(todayTasks)}</div>
      </div>
      ...
      <div className="flex flex-row mt-[45px] gap-[40px]">
        ...
        <div className="border-2 w-[470px] h-[260px] rounded-4xl p-6 flex flex-col">
          ...
          <div className="flex-1 overflow-hidden">{renderTasks(weekTasks)}</div>
        </div>
      </div>
    </div>
  );
